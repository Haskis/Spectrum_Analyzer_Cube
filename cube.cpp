...
Cube::Cube(QObject *parent) :
    QObject(parent)
{
    port=new QextSerialPort(QextSerialPort::EventDriven,this); //Make new QextSerialPort then close it
    port->close();
...
void Cube::initSerialPort(QString string) //Serial port initialization, same as on led Cube
{
    delete port;
